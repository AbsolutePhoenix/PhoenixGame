...
import games.absolutephoenix.phoenixgame.reference.Reference;
...
public class Network extends Game {
	...
	public static void findLatestVersion(Action callback) {
		new Thread(() -> {
			...
			try {
				HttpResponse<JsonNode> response = Unirest.get("https://api.github.com/repos/AbsolutePhoenix/PhoenixGame/releases").asJson();
				if (response.getStatus() != 200) {
					...
					latestVersion = new VersionInfo(new Version(Reference.VERSION), "", "");
				} else {
