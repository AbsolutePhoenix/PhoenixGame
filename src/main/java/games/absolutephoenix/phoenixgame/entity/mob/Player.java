...
import games.absolutephoenix.phoenixgame.entity.ItemHolder;
...
import games.absolutephoenix.phoenixgame.entity.furniture.Tnt;
...
import games.absolutephoenix.phoenixgame.gfx.Rectangle;
...
import games.absolutephoenix.phoenixgame.network.MinicraftServerThread;
import games.absolutephoenix.phoenixgame.saveload.Save;
import games.absolutephoenix.phoenixgame.screen.CraftingDisplay;
import games.absolutephoenix.phoenixgame.screen.InfoDisplay;
import games.absolutephoenix.phoenixgame.screen.LoadingDisplay;
import games.absolutephoenix.phoenixgame.screen.PauseDisplay;
import games.absolutephoenix.phoenixgame.screen.PlayerInvDisplay;
		...
		if (activeItem != null) {
			...
			if (t.x >= 0 && t.y >= 0 && t.x < level.w && t.y < level.h) { // if the target coordinates are a valid tile...
				...
				if (Game.isValidServer() && this instanceof RemotePlayer) {// only do this if no interaction was actually made; b/c a tile update packet will generally happen then anyway.
					MinicraftServerThread thread = Game.server.getAssociatedThread((RemotePlayer)this);
					//if(thread != null)
